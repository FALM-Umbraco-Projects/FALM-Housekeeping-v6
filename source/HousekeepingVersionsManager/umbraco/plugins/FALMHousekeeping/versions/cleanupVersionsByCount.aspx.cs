...
namespace FALMHousekeepingVersionsManager
{
	public partial class cleanupVersionsByCount : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				lblWarning.Text = "<p><strong><em>" + getDictionaryItem("versions_CleanupByCount_PageWarning") + "</em></strong></p><br /><br />";
			}
		...
		protected void btnClearVersions_Click(object sender, EventArgs e)
		{
			...
			strSQLGetVersions  = @"SELECT nodeId, published, documentUser, versionId, text, releaseDate, expireDate, updateDate, templateId, alias, newest into #tmp 
									...
									JOIN cmsDocument DD ON CV.versionId = DD.versionId WHERE DD.nodeId = D.nodeId) AS tmp 
									WHERE tmp.RowNum <= @versionsToKeep OR tmp.published = 1 OR tmp.newest = 1)) 
